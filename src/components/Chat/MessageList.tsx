...
const MessageBubble = ({ message, onFeedback }: { message: SidekickMessage, onFeedback: (id: string, isHelpful: boolean) => void }) => {
    ...
    const hasContent = message.text || message.taskCards?.length || message.diagnostics || message.chartData || message.alertCard || message.richMenuGuidance || message.sizeConsultant || message.proactiveAlert || message.analyticsReport || message.action || message.suggestions?.length;
